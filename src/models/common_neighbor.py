...
def commonNeighbor(G, ebunch=None):
    result = []
    if (ebunch is None):
        raise ValueError("Không thể chạy với ebunch rỗng.")
    for (u, v) in ebunch:
        cns = list(nx.common_neighbors(G, u, v))
        result.append((u, v, len(cns)))
    return result
...
class CommonNeighborThread(Thread):
    def __init__(self,
                 lock,
                 G,
                 lstNodes: list,
...
class CommonNeighbor(Algorithm):
    def __init__(self,
                 id=None,
        ...
        super().__init__(name="CommonNeighbor", id=id, setting={},
